...
/**
 * Update Lead Status
 * Timeline mein entry push karega
 */
...
const convertLead = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, lead, "Lead converted successfully")
    );
});
...
const resolveComplaint = asyncHandler(async (req, res) => {
    ...
    lead.timeline.push({
        ...
        updatedBy:     req.user.name,
        updatedByRole: req.user.role,
        queryResolved: queryResolved || true,
        createdAt:     new Date()
    });
    ...
    return res.status(200).json(
        new ApiResponse(200, lead, "Complaint resolved")
    );
});
